...
client.on('ready', async () => {
  ...
  console.log(`Made by Saizuo`)
  console.log(`Credits Joe Mama | https://discord.gg/G9USQwbhUm`)
  console.log(`Here`)
  client.user.setActivity(config.status.name, { type: config.status.type.toUpperCase(), url: "https://twitch.tv/JoeMama" })
});
...
> \`${prefix}setstaff\` - set staff roles`)
  ...
  if (command == prefix + 'setstaff'){
    ...
    if (args.length != 2) return message.lineReply({ embed: { description: `Please provide an Admin role id, *then* a Mod role id with this command! `, color: `PURPLE` } })
    if (message.mentions.roles.length < 2 && !Number(args[0]) && !Number(args[1])) return message.lineReply({ embed: { description: `Please mention an Admin role (or iD) first, *then* a Mod role (or iD) with this command! `, color: `#PURPLE` } })
  ...
  if (command == prefix + 'send' || command == prefix + 'ticket') {
    ...
    if (!sfas || sfas === null) return message.lineReply({ embed: { description: `This server needs to set up their ticket channels first! \`${prefix}setchannels\``, color: `PURPLE` } })
